...
You will love it.`)
	...
	is.Equal(len(greetOutputObject.Fields), 1)
	...
	is.Equal(welcomeInputObject.Fields[2].Type.SwiftType, "Int")
	...
	is.Equal(len(welcomeOutputObject.Fields), 1)
...
func TestNoDefaultErrorField(t *testing.T) {
	is := is.New(t)
	parser := New("./testdata/services/pleasantries")
	def, err := parser.Parse()
	is.NoErr(err)

	for _, service := range def.Services {
		for _, method := range service.Methods {
			obj, err := def.Object(method.OutputObject.TypeName)
			is.NoErr(err)
			for _, field := range obj.Fields {
				if field.Name == "Error" {
					t.Fatalf("unexpected Error field on %s.%s output", service.Name, method.Name)
				}
			}
		}
	}
...
func TestMethodsByMetadata(t *testing.T) {
	...
	is.Equal(methodsByMetadata[0].Metadata["path"], "/api/v1/endpoint-1")
	is.Equal(len(methodsByMetadata[0].Methods), 1)
	is.Equal(methodsByMetadata[0].Methods[0].Name, "three")
	is.Equal(methodsByMetadata[1].Metadata["path"], "/api/v1/endpoint-2")
